...
#TODO: Fix error [b'{"detail":"Authentication credentials were not provided."}']
...
class RouteCreateTest(APITestCase):
    ...
    def test_create_successfully(self):
        ...
        request = factory.post(self.url_create)

        force_authenticate(request, self.user, token=self.user.auth_token)
        response = self.client.post(self.url_create, data,  format='json')
        self.assertEquals(response.status_code, 201)
